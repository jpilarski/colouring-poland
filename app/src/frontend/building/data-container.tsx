...
const withCopyEdit: (wc: React.ComponentType<CategoryViewProps>) => DataContainerType = (WrappedComponent: React.ComponentType<CategoryViewProps>) => {
    return class DataContainer extends React.Component<DataContainerProps, DataContainerState> {
        ...
        static getDerivedStateFromProps(props, state): DataContainerState {
            const newBuildingId = props.building == undefined ? undefined : props.building.lokalizacja_id_budynku;
            ...
            for (let key in dataFields) {
                ...
                if (props.cat == 'team' && fieldName != null && !blackListedKeys.includes(key)) {
                    categoryKeys[key] = true;
                }
            }
        ...
        async doSubmit(edits: Partial<Building & BuildingUserAttributes>) {
            ...
            try {
                const buildingUpdate = await sendBuildingUpdate(this.props.building.lokalizacja_id_budynku, edits);
                const updatedBuilding = Object.assign({}, this.props.building, buildingUpdate);
                this.props.onBuildingUpdate(this.props.building.lokalizacja_id_budynku, updatedBuilding);
            } catch (error) {
                this.setState({ error });
            }
        }
        ...
        async handleVerify(slug: string, verify: boolean, x: number, y: number) {
            ...
            try {
                const data = await apiPost(
                    `/api/buildings/${this.props.building.lokalizacja_id_budynku}/verify.json`,
                    verifyPatch
                );
                ...
                if (data.error) {
                    ...
                    this.props.onUserVerifiedUpdate(this.props.building.lokalizacja_id_budynku, data);
                }
            } catch (err) {
        ...
        render() {
            ...
            const headerBackLink = `/${this.props.mode}/categories${this.props.building != undefined ? `/${this.props.building.lokalizacja_id_budynku}` : ''}`;
                                        ...
                                        !copy.copying ?
                                            <>
                                                <NavLink
                                                    ...
                                                    to={`/${this.props.mode}/${this.props.cat}/${this.props.building.lokalizacja_id_budynku}/history`}
                        ...
                        />
                        {
                            this.props.inactive ?
                                ...
                                this.props.building != undefined ?
                                    <form
                                        action={`/edit/${this.props.cat}/${this.props.building.lokalizacja_id_budynku}`}
                                        method="POST"
